...
mapped_lvl3_bugFix <- tar_map(
       ...
       values = tibble(
         method_function = rlang::syms("setupHydrography"),
         huc4 = lookUpTable[lookUpTable$level == 3 & lookUpTable$HUC4 %in% c('1304', '1702'),]$HUC4), #ignore terminal basins that need too be split to facilitate 
       names = "huc4",
       tar_target(hydrography, method_function(path_to_data, huc4),
                                              resources = tar_resources(future = tar_resources_future(plan = tweak(batchtools_slurm,template = "slurm_future.tmpl",resources = list(num_cores = cores_req))))), #prep hydrography for routing
       tar_target(calibratedParameters, grabCalibratedParameters_from_logs(huc4)),
...
mapped_lvl12 <- tar_map(
      ...
      #                                                        myPopSize, mymaxIter, myRun, maxFitness, mutationRate, cores),
      #                                        resources = tar_resources(future = tar_resources_future(plan = tweak(batchtools_slurm,template = "slurm_future.tmpl",resources = list(num_cores = cores_req))))), #specify cores wildcard for calibration
       tar_target(calibratedParameters, grabCalibratedParameters_from_logs(huc4)), #SINGLE BASIN, SO NO NEED FOR BUG FIX STUFF, JUST CALL THIS VERSION OF THE TARGET
...
mapped_1710a <- tar_map(
       ...
       tar_target(calibratedParameters, calibrateModelWrapper(hydrography, substr(sub,1,4), glorich_data, C_groundwater, C_atmosphere, emergenceQ, NA, #calibrate model to raymond CO2
                                              ...
                                              cue = tar_cue(mode = "never"),
                                              resources = tar_resources(future = tar_resources_future(plan = tweak(batchtools_slurm,template = "slurm_future.tmpl",resources = list(num_cores = cores_req))))), #specify cores wildcard for calibration
       tar_target(final, runModel(hydrography, calibratedParameters, C_groundwater, C_atmosphere, substr(sub,1,4), emergenceQ, NA),cue = tar_cue(mode = "never")), #run final version of model
...
list(
  ...
  #COMBINE ALL BY-BASIN TARGETS OF IMPORTANCE----------------------------------------------------------
  ...
  tar_target(combined_sources_basin, fixCombo0427(combined_sources_basin_init)),#double counts 0427 b/c of 0427_1: it exists only because the basin actually exports into two downstream basins (preserved only for exportedCO2). Fixed below.
  ...
  #combine all model basinLakeProperties targets (levels that have currently finished)
  ...
  #combine all model maps (levels that have currently finished)
  ...
  tar_target(summaryObject, gatherResults(combined_emissions, combined_sources_basin, combined_uncertainty, combined_basinProperties, combined_basinLakeProperties, combined_sources_by_order,
                                          list(lumped_01, lumped_02, lumped_03, lumped_04, lumped_05, lumped_06, lumped_07, lumped_08, lumped_09, lumped_10, lumped_11, lumped_12, lumped_13, lumped_14, lumped_15, lumped_16, lumped_17, lumped_18))),
