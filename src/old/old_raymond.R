...
runRaymondModel_old <- function(path_to_data, HUC2, glorich_data,hydrographyList,emissions_uncertainty_list) {
  ...
  yield <- exp(runoff[1,]$Ln.Precipitation...mm.day.1.)/86400*0.001 #[m/s]
 ...
 # hydrosheds <- dplyr::filter(results, waterbody == 'River')
 # hydrosheds$ORD_STRA <- hydrosheds$StreamOrde
  lakes <- dplyr::filter(results, waterbody == 'Lake/Reservoir')
...
#  lakeCO2 <- median(results[results$waterbody == 'Lake/Reservoir',]$CO2_ppm) #use calibrated value to remove uncertainties from CO2s
  ...
  #our model results--------------------------------------
  results$FCO2_gC_yr <- ifelse(results$waterbody == 'River',
                               results$FCO2_gC_m2_yr * results$W_m * results$LengthKM * 1000, #river g-C/yr
                               results$FCO2_gC_m2_yr * results$lakeSA_m2) #lake/reservoir g-C/yr

  #aggregate over watershed
  networkOutput <- dplyr::group_by(results, waterbody) %>%
