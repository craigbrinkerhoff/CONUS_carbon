...
runLumpedModels <- function(HUC2, glorich_data,hydrographyList,co2List,emissions_list,emissions_uncertainty_list) {
  ...
  model_emissions <- do.call("rbind", emissions_list) #make HUC2 emissions df
  cal_uncertainty <- do.call("rbind", emissions_uncertainty_list) #make HUC2 uncertainty estimates df
  ...
  #UPSCALING APPROACH CALCULATION-------------------------------------
  #RIVERS---------------------------------------
  ...
  #LUMPED CO2 MODEL
  network$lumpedCO2_FCO2_gC_m2_yr <- ifelse(network$waterbody == 'River', ((riverCO2-400)*network$henry*1e-6)*(network$k_co2*network$D)*(1/0.001)*12.01*(60*60*24*365), ((lakeCO2-400)*network$henry*1e-6)*(network$k_co2*network$D)*(1/0.001)*12.01*(60*60*24*365)) #[g-C/m2/yr]
  network$lumpedCO2_FCO2_gC_yr <- ifelse(network$waterbody == 'River', network$lumpedCO2_FCO2_gC_m2_yr*network$W*network$LengthKM*1000, network$lumpedCO2_FCO2_gC_m2_yr*network$frac_lakeSurfaceArea_m2) #[g-C/yr]

  #LUMPED k MODEL
  model_co2$lumpedK_FCO2_gC_m2_yr <- ifelse(model_co2$waterbody == 'River', ((model_co2$CO2_ppm-400)*henry*1e-6)*(rivers_k_co2_lumped_m_s)*(1/0.001)*12.01*(60*60*24*365), ((model_co2$CO2_ppm-400)*henry*1e-6)*(model_co2$k_co2_m_s)*(1/0.001)*12.01*(60*60*24*365)) #[g-C/m2/yr]
  model_co2$lumpedK_FCO2_gC_yr <- ifelse(model_co2$waterbody == 'River', model_co2$lumpedK_FCO2_gC_m2_yr*model_co2$W_m*model_co2$LengthKM*1000, model_co2$lumpedK_FCO2_gC_m2_yr*model_co2$lakeSA_m2) #[g-C/yr]


  #OUR MODEL--------------------------------------
  networkOutput <- dplyr::group_by(model_emissions, waterbody) %>% #summ over whole basin
                       dplyr::summarise(sumFCO2_TgC_yr = sum(sumFCO2_TgC_yr, na.rm=T), #[Tg-C/yr]
  ...
  network_fluxes_lumped <- data.frame('huc2'=HUC2,
                                  'sumFCO2_lumped_TgC_yr' = c(rivers_FCO2_lumped_total, lakes_FCO2_lumped_total), #[Tg-C/yr]
                                  'sumFCO2_lumped_co2_TgC_yr'=c(sum(network[network$waterbody == 'River',]$lumpedCO2_FCO2_gC_yr, na.rm=T)*1e-12, sum(network[network$waterbody == 'Lake/Reservoir',]$lumpedCO2_FCO2_gC_yr, na.rm=T)*1e-12), #[Tg-C/yr]
                                  'sumFCO2_lumped_k_TgC_yr'=c(sum(model_co2[model_co2$waterbody == 'River',]$lumpedK_FCO2_gC_yr, na.rm=T)*1e-12, sum(model_co2[model_co2$waterbody == 'Lake/Reservoir',]$lumpedK_FCO2_gC_yr, na.rm=T)*1e-12),#[Tg-C/yr]                         
                                  'sumFCO2_TgC_yr'=c(networkOutput[networkOutput$waterbody == 'River',]$sumFCO2_TgC_yr, networkOutput[networkOutput$waterbody == 'Lake/Reservoir',]$sumFCO2_TgC_yr), #[Tg-C/yr]
                                  'sumFCO2_conus_TgC_yr'=c(networkOutput[networkOutput$waterbody == 'River',]$sumFCO2_conus_TgC_yr, networkOutput[networkOutput$waterbody == 'Lake/Reservoir',]$sumFCO2_conus_TgC_yr), #[Tg-C/yr]
